The usage of `<annotative-code-full />` is nearly identical to `<annotative-code />`,
...
<annotative-code-full /> supports all [190+ programming languages](https://github.com/highlightjs/highlight.js/tree/main/src/languages) of highlight.js.
However, it comes with a cost that the bundle size is larger, which is ~932.6k (gzipped: 295.4k) - according to [Import Cost](https://marketplace.visualstudio.com/items?itemName=wix.vscode-import-cost)
